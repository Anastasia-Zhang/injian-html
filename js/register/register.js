...
(function($) {
	...
	require.prototype = {
		_init: function(b) {
			...
			$('body').on({
				blur: function(a) {
					...
					if (b.phone && $(this).attr("id") === "telphone") d._change($(this));
				...
				keyup: function(a) {
					if (b.telphone && $(this).attr("id") === "telphone") d._change($(this))
				},
				change: function(a) {
		...
		_sendVerify: function() {
			var a = this;
			$("#getotp").text("发送验证码").hide();
			$("#time_box").text("10 s后可重发").show();
			if (timerC === 0) {
				clearTimeout(h);
				timerC = 60;
				var b = /^1([^01269])\d{9}$/;
				if (!b.test($("#telphone").val())) {
					$("#time_box").text("发送验证码")
				} else {
					$("#getotp").show();
					$("#time_box").hide()
		...
		_change: function(a) {
			...
			if (a.val().length != 11) {
				...
				$("#otp").val("");
				this.config.clearTips ? this.config.clearTips($("#otp")) : j._clearTips($("#otp"));
				return
			}
			var c = /^1([^01269])\d{9}$/;
...
jQuery(document).ready(function(){
	...
	$("#btn_part2").on("click",function(){
		...
		$.ajax({
			type:"POST",
			contentType:"application/x-www-form-urlencoded",
			url:"http://localhost:8070/user/register",
			data:{
				"telphone":$("#telphone").val(),
			...
			success:function(data){
				if(data.status =="success"){
					alert("注册成功");
					userInfo = data.data;
					$("#useRegisterName").text(userInfo.username);
					if(!verifyCheck._click()) return;
					$(".part2").hide();
					$(".part4").show();
